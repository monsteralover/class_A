...
<div id="wrap">
   ...
   <div id="container">
	...
	<ul>
		<li><c:out value="${requestScope.name }"/>(<c:out value="${param.name }"/>)</li>
		<li><c:if test="${not empty type }">
			<c:forEach var="type"  items="${type }">
			<c:out value="${type }"/>
			</c:forEach>
		</c:if>
		<c:if test="${empty type }">파일 타입을 선택하지 않으셨습니다.</c:if>
		</li>
		<li>file1: ${file1.origin }(${file1_rename })</li>
		<li>file2: ${file2.origin }(${file2_rename })</li>
	</ul>
	<a href="day0916/use_fileupload.html">업로드폼</a>
