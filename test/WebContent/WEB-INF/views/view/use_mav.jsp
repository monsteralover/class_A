<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"
    info="ModelAndView객체에 할당된 값을 받아 화면을 만드는 일"
    %>
...
<div id="wrap">
   ...
   <div id="container">
    ...
    <select>
    	<option>----능력단위 선택-----</option>
    	<c:forEach var="subject" items="${view_data}">
    	<option value="${subject }"><c:out value="${subject}"/></option>
    	</c:forEach>
    </select>
    <br/>
  <%--   <c:out value="${ requestScope.name_data}"/> --%>
	<c:forTokens items="${name_data}" delims=","  var="name">
	<input type="checkbox" name="name" value="${name}"/> <c:out value="${name }"/>
	</c:forTokens>  
